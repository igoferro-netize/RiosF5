"""
Modelo de Empresa Completo com Controle de Bloqueio
...
class Empresa(db.Model):
    ...
    # Status e Bloqueio por Inadimplência
    status = db.Column(db.String(20), default='ativo')  # ativo, bloqueado, suspenso, cancelado, trial
    ativo = db.Column(db.Boolean, default=True)
    bloqueado_por_inadimplencia = db.Column(db.Boolean, default=False)
    ...
    def to_dict(self):
        return {
            ...
            'status': self.status,
            'ativo': self.ativo,
            'bloqueado_por_inadimplencia': self.bloqueado_por_inadimplencia,
    ...
    def is_active(self):
        """Verifica se a empresa está ativa (não bloqueada)"""
        return self.ativo and self.status == 'ativo' and not self.bloqueado_por_inadimplencia
    
    def bloquear(self, motivo="Inadimplência"):
        """Bloqueia a empresa"""
        self.bloqueado_por_inadimplencia = True
        self.status = 'bloqueado'
        self.motivo_bloqueio = motivo
        self.data_bloqueio = datetime.utcnow()
    
    def desbloquear(self):
        """Desbloqueia a empresa"""
        self.bloqueado_por_inadimplencia = False
        self.status = 'ativo'
